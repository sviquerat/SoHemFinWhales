#### various functions related to the beta distribution ####

samplePlotBeta<-function(q25,q75,threshold,...){ #obsolete
  p<-drawFromBeta(q25,q75,10000)
  pdf<-density(p$samples,from=0,to=1)
  coords<-threshHoldBetaDensity(pdf,0,threshold)
  plot(pdf,main='',...)
  polygon(coords$below,col='orange')
  polygon(coords$above,col='lightblue')
  abline(v=threshold,col='purple',lty=2,lwd=1.5)
  abline(v=c(q25,q75),col='red',lty=2,lwd=1.5)
  legend('topright',legend=paste0(100*round(1-p$ECDF(threshold),4),'%'),box.col='lightblue')
  legend('topleft',legend=paste0(100*round(p$ECDF(threshold),4),'%'),box.col='orange')
}

estimateBeta<-function(q25,q75,...){
  return(beta.parms.from.quantiles(c(q25,q75),c(0.25,0.75),plot=F,...))
}

drawFromBeta<-function(q25,q75,N=1000,safetyDistance=2E-3,...){
  if(q25 == q75 || abs(q75-q25) < safetyDistance || q75 == 1){
    avg<-(q25+q75)/2
    samples<-rep(avg,N)
    ECDF<-function(x){
      value<-1
      D<-c(q25,q75)-x
      if( all(D>0) ){return(0)} #both are larger than x
      if( all(D<0) ) {return(1)} #both are smaller than x
      if( D[1]<0 ) {return(abs(x-q25))} #q25 is smaller than x, q75 is not
    }
    return(list(samples=samples,ECDF=ECDF))
  }
  t<-estimateBeta(q25,q75,...)
  samples<-rbeta(N,t$a,t$b)
  ECDF <- ecdf(samples)
  return(list(samples=samples,ECDF=ECDF))
}

threshHoldBetaDensity<-function(pdf,x1=0,x2=.5,x3=1,...){ #duplicate in MERGE_settings.R
  y<-pdf$y #cumulative probabilities
  x<-pdf$x #probabilities
  pos<-which(x>=x1 & x<x2)
  Y1<-y[pos]
  X1<-x[pos]
  pos2<-which(x>=x2 & x<=x3)
  Y2<-y[pos2]
  X2<-x[pos2]
  coords1=list(x=c(X1,rev(X1)),y=c(Y1,rep(0,length(pos))))
  coords2=list(x=c(X2,rev(X2)),y=c(Y2,rep(0,length(pos2))))
  return(list(below=coords1,above=coords2))
}
